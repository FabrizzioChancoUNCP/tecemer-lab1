...
def sigmoide_matematica(x):
    # Matemáticamente: 1 / (1 + e^-x)
    return 1 / (1 + np.exp(-x))
...
def main():
    ...
    y_math = sigmoide_matematica(x)
    print("--- Evaluación Matemática (NumPy) ---")
    print(f"Entrada 0: {0.0}")
    print(f"Salida Sigmoide (debe ser 0.5): {sigmoide_matematica(0.0)}\n")
    ...
    modelo.summary()
    graficar_neurona("Sigmoide", "sigmoid_neurona.png")

    plt.figure(figsize=(8, 5))
    plt.plot(x, y_math, label="Sigmoide (NumPy)", color='purple', linewidth=2)
    plt.title("Función de Activación: Sigmoide")
    plt.xlabel("Entrada (x)")
    plt.ylabel("Salida (y - Probabilidad)")
    ...
    print("Gráfico guardado como 'sigmoid_plot.png'")
